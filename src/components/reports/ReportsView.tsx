...
export type DatePeriod = 'TODAY' | 'YESTERDAY' | 'WEEK' | 'MONTH' | 'YEAR' | 'ALL' | 'CUSTOM';
...
export const ReportsView: React.FC = () => {
  ...
  const totalStockValuation = useMemo(
    () => roundMoney(products.reduce((sum, p) => sum + (p.current_stock > 0 ? p.current_stock * p.average_cost : 0), 0)),
    [products]
  );
  const totalRetailPotential = useMemo(
    () => roundMoney(products.reduce((sum, p) => sum + (p.current_stock > 0 ? p.current_stock * p.selling_price : 0), 0)),
    [products]
  );
  const lowStockItems = useMemo(
    () => products.filter((p) => p.track_stock && p.current_stock <= p.min_stock_threshold),
    [products]
  );

  // Top Products & Services Ranking
  const topItemsRanking = useMemo(() => {
    const itemMap: Record<string, { id: string; name: string; type: string; qty: number; revenue: number; profit: number }> = {};
    filteredSales.forEach((s) => {
      s.items.forEach((it) => {
        ...
        itemMap[it.item_id].revenue = roundMoney(itemMap[it.item_id].revenue + it.total);
        itemMap[it.item_id].profit = roundMoney(itemMap[it.item_id].profit + it.gross_profit);
      });
    });
  ...
  const handleExportCSV = () => {
    ...
    if (activeTab === 'PL') {
      ...
      const headers = ['Invoice #', 'Date Time', 'Customer', 'Cashier', 'Payment Method', 'Items Count', 'Discount', 'Tax', 'Grand Total', 'COGS', 'Gross Profit'];
      ...
      const headers = ['Date', 'Category', 'Account', 'Description', 'Reference #', 'Entered By', 'Amount'];
      ...
      const headers = ['SKU', 'Product Name', 'Category', 'Unit', 'Purchase Cost', 'Selling Price', 'Current Stock', 'Stock Value (WAC)', 'Status'];
      ...
      const headers = ['Closing Date', 'Opening Cash', 'Cash Sales', 'Cash Expenses', 'Transfers In', 'Transfers Out', 'Expected Cash', 'Actual Counted', 'Difference', 'Closed By'];
    ...
    <div className="flex-1 overflow-y-auto p-4 sm:p-6 space-y-5 bg-slate-950 select-none text-slate-100">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-3 pb-3 border-b border-slate-800 no-print">
        <div>
          ...
          <p className="text-xs text-slate-400 mt-0.5">
            Audit-reconciled statements, stock valuation, and cashier performance for {organization.name}.
          </p>
        </div>
      ...
      <div className="flex gap-1.5 overflow-x-auto pb-1 border-b border-slate-800/80 no-print text-xs">
        {[
          ...
          { id: 'PAYMENTS', label: 'Payment Tenders & Cash Flow', icon: Wallet },
      ...
      {activeTab === 'PL' && (
        <Card className="p-6 bg-slate-900/90 border-slate-800 max-w-4xl mx-auto space-y-6">
          <div className="text-center pb-4 border-b border-slate-800">
            ...
            <p className="text-xs text-slate-400">Formal Statement of Profit and Loss</p>
          ...
          <div className="space-y-2">
            ...
            <div className="pl-4 space-y-1 text-xs text-slate-400">
              {expenseByCategory.length === 0 ? (
                <p className="italic text-slate-500">No overhead expenses recorded for this period.</p>
              ) : (
                ...
                {filteredSales.length === 0 ? (
                  ...
                  filteredSales.map((s) => (
                    <tr key={s.id} className="hover:bg-slate-900/60 transition-colors">
                      ...
                      <td className="py-2.5 px-3 text-slate-400">{s.cashier_name}</td>
                ...
                {filteredExpenses.length === 0 ? (
                  ...
                  filteredExpenses.map((e) => (
                    <tr key={e.id} className="hover:bg-slate-900/60 transition-colors">
                      <td className="py-2.5 px-3 font-mono text-slate-400">{formatDate(e.date)}</td>
                      <td className="py-2.5 px-3 font-semibold text-slate-200">
                        {e.category_name}
                      </td>
                      <td className="py-2.5 px-3 text-slate-400">{e.account_name}</td>
                      <td className="py-2.5 px-3 text-slate-300">{e.description}</td>
                      <td className="py-2.5 px-3 font-mono text-slate-500">{e.reference_number || '-'}</td>
                      <td className="py-2.5 px-3 text-slate-400">{e.entered_by}</td>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
            <Card className="p-3.5 bg-slate-900 border-slate-800">
              ...
              <p className="text-xl font-mono font-bold text-cyan-400 mt-1">
                {formatCurrency(totalStockValuation, organization.currency_symbol)}
              </p>
            </Card>
            ...
            <Card className="p-3.5 bg-slate-900 border-slate-800">
              ...
              <p className="text-xl font-mono font-bold text-emerald-400 mt-1">
                {formatCurrency(totalRetailPotential, organization.currency_symbol)}
              </p>
            </Card>
              ...
              <table className="w-full text-left text-xs border-collapse">
                <thead>
                  <tr className="border-b border-slate-800 bg-slate-900/90 text-slate-400 uppercase font-mono text-[10px]">
                    ...
                    <th className="py-2.5 px-3 text-right">Average Cost (WAC)</th>
                  ...
                  filteredMovements.map((m) => {
                    ...
                    return (
                      <tr key={m.id} className="hover:bg-slate-900/60 transition-colors">
                        ...
                        <td className="py-2.5 px-3 text-slate-400 text-[11px]">{m.notes || '-'}</td>
                      </tr>
                    );
                  })
                )}
            ...
            <table className="w-full text-left text-xs border-collapse">
              <thead>
                <tr className="border-b border-slate-800 bg-slate-900/90 text-slate-400 uppercase font-mono text-[10px]">
                  ...
                  <th className="py-2.5 px-3 text-right">Gross Profit Contribution</th>
                </tr>
              </thead>
              <tbody className="divide-y divide-slate-800/60 font-sans">
                {topItemsRanking.length === 0 ? (
                  ...
                  topItemsRanking.map((it, idx) => (
                    <tr key={it.id} className="hover:bg-slate-900/60 transition-colors">
                      ...
                      <td className="py-2.5 px-3 font-semibold text-slate-200">{it.name}</td>
                      <td className="py-2.5 px-3">
                        <Badge variant={it.type === 'SERVICE' ? 'cyan' : 'emerald'} size="sm">
                          {it.type}
                        </Badge>
                      </td>
                      <td className="py-2.5 px-3 text-right font-mono font-bold text-slate-100">
                      ...
                      <td className="py-2.5 px-3 text-right font-mono font-bold text-cyan-400">
                        {formatCurrency(it.revenue, organization.currency_symbol)}
                      </td>
                      <td className="py-2.5 px-3 text-right font-mono font-semibold text-emerald-400">
                        {formatCurrency(it.profit, organization.currency_symbol)}
                      </td>
                    </tr>
                  ))
                )}
        ...
        <div className="space-y-4">
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-3">
            {paymentMethodsBreakdown.map((pm) => (
              <Card key={pm.name} className="p-3.5 bg-slate-900 border-slate-800">
                <span className="text-[11px] font-semibold text-slate-400 uppercase tracking-wider block">
                  {pm.name}
                </span>
                <p className="text-xl font-mono font-bold text-slate-100 mt-1">
                ...
                <tbody className="divide-y divide-slate-800/60 font-sans">
                  {accounts.map((acc) => (
                    <tr key={acc.id} className="hover:bg-slate-900/60 transition-colors">
                      ...
                      <td className="py-2.5 px-3 text-right font-mono text-slate-400">
                        {formatCurrency(acc.opening_balance, organization.currency_symbol)}
                      </td>
            ...
            <table className="w-full text-left text-xs border-collapse">
              <thead>
                <tr className="border-b border-slate-800 bg-slate-900/90 text-slate-400 uppercase font-mono text-[10px]">
                  ...
                  <th className="py-2.5 px-3 text-right">Outstanding Balance / Credit</th>
                </tr>
              </thead>
              <tbody className="divide-y divide-slate-800/60 font-sans">
                {customers.length === 0 ? (
                  ...
                  customers.map((c) => (
                    <tr key={c.id} className="hover:bg-slate-900/60 transition-colors">
                      ...
                      <td className="py-2.5 px-3 text-slate-400 text-[11px]">{c.address || '-'}</td>
                      <td className="py-2.5 px-3 text-right font-mono font-bold text-slate-100">
                        {formatCurrency(c.total_purchases, organization.currency_symbol)}
                      </td>
                      <td className="py-2.5 px-3 text-right font-mono font-bold text-amber-400">
                        {formatCurrency(c.outstanding_balance, organization.currency_symbol)}
                      </td>
                    </tr>
                  ))
                )}
            ...
            <table className="w-full text-left text-xs border-collapse">
              <thead>
                <tr className="border-b border-slate-800 bg-slate-900/90 text-slate-400 uppercase font-mono text-[10px]">
                  ...
                  <th className="py-2.5 px-3 text-right">Expected Drawer Cash</th>
                  <th className="py-2.5 px-3 text-right">Actual Counted</th>
                  <th className="py-2.5 px-3 text-right">Variance / Difference</th>
              ...
              <tbody className="divide-y divide-slate-800/60 font-sans">
                {filteredClosings.length === 0 ? (
                  ...
                  filteredClosings.map((cl) => {
                    ...
                    return (
                      <tr key={cl.id} className="hover:bg-slate-900/60 transition-colors">
                        <td className="py-2.5 px-3 font-mono font-bold text-slate-200">
                          {cl.closing_date}
                        </td>
                        <td className="py-2.5 px-3 text-right font-mono text-slate-400">
                          {formatCurrency(cl.opening_cash, organization.currency_symbol)}
                        </td>
                        <td className="py-2.5 px-3 text-right font-mono text-emerald-400">
                          {formatCurrency(cl.cash_sales, organization.currency_symbol)}
                        </td>
                        <td className="py-2.5 px-3 text-right font-mono text-rose-400">
                          {formatCurrency(cl.cash_expenses, organization.currency_symbol)}
                        </td>
                        <td className="py-2.5 px-3 text-right font-mono text-slate-200 font-semibold">
                          {formatCurrency(cl.expected_cash, organization.currency_symbol)}
                        </td>
                        <td className="py-2.5 px-3 text-right font-mono text-cyan-300 font-bold">
                          {formatCurrency(cl.actual_cash, organization.currency_symbol)}
                        </td>
                        <td className="py-2.5 px-3 text-right font-mono font-bold">
                          <span
                            className={
                          ...
                          >
                            {cl.difference > 0 ? `+${cl.difference}` : cl.difference}
                          </span>
                        </td>
                        <td className="py-2.5 px-3 text-slate-300">{cl.closed_by}</td>

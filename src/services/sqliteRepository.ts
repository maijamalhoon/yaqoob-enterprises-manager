...
export class SQLiteRepository {
  ...
  async getProducts(organizationId: string): Promise<Product[]> { requireOrganization(organizationId); return (await (await this.database()).select<Product>('SELECT * FROM products WHERE organization_id = ? ORDER BY name', [organizationId])).filter((product) => product.is_active !== false && product.is_active !== 0); }
